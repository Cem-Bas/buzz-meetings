...
pub struct Config {
    ...
    /// recognized attempt to post to Buzz. Default on (fork change: local
    /// models end turns without publishing, and the local desktop spawn path
    /// never opted in — see README-FORK.md); opt out per agent with
